...
// WICHTIG: Hier die echte Taktrate deines FPGAs eintragen (z.B. 10MHz, 50 MHz, 125 MHz, ....)
...
#define BUFFER_SIZE 6*1024 // Muss eine Potenz von 2 sein (256, 512, 1024) für schnelles Bitmask-Wrapping
...
void send_audio_packet(const uint8_t samples[20]) {
    send_to_fpga('A');
    send_to_fpga('U');
    send_to_fpga('D');
    for(int i = 0; i < 20; i++) {
...
void send_frequency_packet(const uint32_t freqs[20]) {
    send_to_fpga('F');
    send_to_fpga('R');
    send_to_fpga('Q');
    for (int i = 0; i < 20; i++) {
