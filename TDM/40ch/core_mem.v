 ...
 * MODUL: core_mem
 * ---------------------------------------------------------------------
 * Kapselt die Speicherelemente für 10 Kanäle eines TDM-Kerns.
 ...
 * [ÄNDERUNG: Tiefe auf 10 Kanäle verdoppelt]
 */
module core_mem (
    input wire clk,

    // TDM Slot Steuerung
    // [ÄNDERUNG: Slot-Eingänge auf 4 Bit erweitert (0..9)]
    input wire [3:0] slot,      // Aktueller TDM-Slot 
    input wire [3:0] slot_stg1, // Um 1 Takt verzögerter Slot für RAM-Writeback
    
    // MCU RAM-Schreibinterface
    // [ÄNDERUNG: Schreibadresse auf 4 Bit erweitert]
    input wire [3:0]  mcu_waddr,
    ...
    // [ÄNDERUNG: Alle RAM-Arrays auf 10 Elemente (0:9) vergrößert]
    (* ramstyle = "logic" *) reg [7:0]  ram_audio [0:9];
    (* ramstyle = "logic" *) reg [15:0] ram_gain  [0:9];
    (* ramstyle = "logic" *) reg [6:0]  ram_peak  [0:9];
    (* ramstyle = "logic" *) reg [15:0] ram_decay [0:9];
    
    // Große 32-Bit RAMs belassen wir in den physikalischen M4K Blöcken
    (* ramstyle = "M4K" *)   reg [31:0] ram_frq   [0:9];
    (* ramstyle = "M4K" *)   reg [31:0] ram_phase [0:9];

    // Speicher-Initialisierung
    integer i;
    initial begin
        // [ÄNDERUNG: Schleife bis 10 laufen lassen]
