/*
 * MODUL: mcu_rx (TDM-Bus-Version - EMV-Version - 40 KANAL)
 * ---------------------------------------------------------------------
 ...
 * [ÄNDERUNG: Zähler und Busbreiten auf 40 Kanäle skaliert]
 */
module mcu_rx #(
    // Parameter für die Hardware-Härtung
    // [ÄNDERUNG: Timeout auf 100 MHz Systemtakt angepasst]
    ...
    // [ÄNDERUNG: 6-Bit Bus für bis zu 64 Kanäle (wir nutzen 40)]
    output reg [5:0]  w_ch,
    ...
    reg [3:0]  state;
    // [ÄNDERUNG: Zähler auf 8 Bit für 160 Bytes (Frequenz) erweitert]
    reg [7:0]  byte_cnt;
    reg [31:0] temp_freq;
    // [ÄNDERUNG: Zielkanal-Register auf 6-Bit erweitert]
    reg [5:0]  target_ch; 
    reg [15:0] temp_gain;
    ...
    always @(posedge clk) begin
        if (rst) begin
            state        <= S_IDLE;
            byte_cnt     <= 8'd0; // [ÄNDERUNG] 8 Bit Reset
            temp_freq    <= 32'd0;
            temp_gain    <= 16'd0;
            target_ch    <= 6'd0; // [ÄNDERUNG] 6 Bit Reset
            watchdog_cnt <= 32'd0;
            
            w_ch         <= 6'd0; // [ÄNDERUNG] 6 Bit Reset
                ...
                case (state)
                    ...
                    S_F_DATA: begin
                        ...
                        if (byte_cnt[1:0] == 2'b11) begin
                            w_ch       <= byte_cnt[7:2]; // [ÄNDERUNG: Kanal berechnen, obere 6 Bit]
                            w_frq_data <= {temp_freq[23:0], data_in_sync2};
                            w_frq_en   <= 1'b1; 
                        end
                        
                        // [ÄNDERUNG: Stop bei Byte 159 (40 x 4 Bytes)]
                        if (byte_cnt == 8'd159) state <= S_IDLE;
                        else byte_cnt <= byte_cnt + 8'd1;
                    end
                    ...
                    S_G_CH: begin
                        // [ÄNDERUNG: Gültigkeit prüfen: Darf max Kanal 39 sein]
                        if (data_in_sync2 <= 8'd39) begin
                            target_ch <= data_in_sync2[5:0]; // [ÄNDERUNG: 6 Bit Zuweisung]
                            state <= S_G_DATA; byte_cnt <= 8'd0;
                        end else state <= S_IDLE;
                    end
                    
                    S_G_DATA: begin
                        temp_gain <= {temp_gain[7:0], data_in_sync2};
                        if (byte_cnt == 8'd1) begin // [ÄNDERUNG]
                            w_ch        <= target_ch;
                            w_gain_data <= {temp_gain[7:0], data_in_sync2};
                            w_gain_en   <= 1'b1; 
                            state       <= S_IDLE;
                        end else byte_cnt <= byte_cnt + 8'd1;
                    end
